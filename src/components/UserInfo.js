...
class UserInfo extends HTMLElement {
  ...
  connectedCallback () {
    this.wrapper.addEventListener('user-has-been-selected', function (event) {
      const eventForProfile = new Event(event.type)
      eventForProfile.data = event.data
      this.profile.dispatchEvent(eventForProfile)
    }.bind(this))

    this.wrapper.addEventListener('user-has-been-added', function (event) {
    ...
    this.wrapper.addEventListener('user-has-been-deleted', function (event) {
      const eventForNavigator = new Event(event.type)
      eventForNavigator.data = event.data
      this.navigator.dispatchEvent(eventForNavigator)
    }.bind(this))

    this.wrapper.addEventListener('user-has-been-edited', function (event) {
      const eventForNavigator = new Event(event.type)
      eventForNavigator.data = event.data
      this.navigator.dispatchEvent(eventForNavigator)
    }.bind(this))
